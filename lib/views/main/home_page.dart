...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final user = authVM.currentUser; // Kullanıcı nesnesi
    final userName = user?.name ?? "Kullanıcı";

    // 🔥 1. ADIM: BİLDİRİM TERCİHLERİNE GÖRE FİLTRELEME
    // Kullanıcının kapattığı kategoriler listeden anlık olarak süzülür.
    final filteredNotifications = notifVM.notifications.where((n) {
      if (user == null) return true;
      if (n.type == 'saglik' && !(user.preferences['health'] ?? true)) return false;
    ...
    return Scaffold(
        ...
        title: Row(
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () => debugPrint("Ayarlar tıklandı"),
            )
          ],
        ),
      ),
          ...
          children: [
            ...
            Expanded(
              child: filteredNotifications.isEmpty
                  ? const Center(child: Text("Görüntülenecek bildirim yok", style: TextStyle(color: Colors.grey)))
                  : ListView.builder(
                      itemCount: filteredNotifications.length,
                      itemBuilder: (context, index) {
                        final notif = filteredNotifications[index];
                        // 🔥 2. ADIM: KARTIN İÇİNE USER ID GÖNDERİLİYOR
                        return _buildNotificationCard(context, notif, user?.uid);
                      },
                    ),
            ),
          ],
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(context, MaterialPageRoute(builder: (_) => const AddNewNotificationPage()));
        },
      ),
    );
  }

  // 🔔 BİLDİRİM KARTI (GÜNCELLENDİ)
